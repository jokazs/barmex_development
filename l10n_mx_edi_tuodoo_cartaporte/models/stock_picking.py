...
CFDI_XSLT_CADENA = 'l10n_mx_edi_tuodoo_cartaporte/data/cadenaoriginal_cartaporte.xslt'
CFDI_XSLT_CADENA_TFD = 'l10n_mx_edi_tuodoo_cartaporte/data/cadenaoriginal_TFD_1_1.xslt'
ATTACHMENT_NAME = 'CFDI_Carta_Porte_{}.xml'
ATTACHMENT_NAME_CT = 'CFDI_Carta_Traslado_{}.xml'
...
class Picking(models.Model):
    ...
    l10n_mx_edi_operador_id = fields.Many2one(
        comodel_name='l10n_mx_edi.operador',
        string='Operador',
        help='The operator associated with the vehicle used for Federal Transport')
    ...
    def _l10n_mx_edi_create_delivery_guide(self):
        ...
        for record in self:
            ...
            xml = self.env['ir.qweb'].render(self.env.ref('l10n_mx_edi_tuodoo_cartaporte.cfdi_cartaporte'), values)
    ...
    def l10n_mx_edi_action_send_delivery_guide(self):
        ...
        for record in self:
            ...
            record.l10n_mx_edi_cfdi_name = cfdi_attachment.name
    ...
    @api.depends('l10n_mx_edi_cfdi_name', 'l10n_mx_edi_status')
    def _compute_cfdi_values(self):
        ...
        for inv in self:
            ...
            _logger.info("Este es attachment_id")
            _logger.info(attachment_id)
            _logger.info("Este es attachment_id.store_fname")
            _logger.info(attachment_id.store_fname)
    ...
    l10n_mx_edi_content_ct = fields.Binary(compute='_l10n_mx_edi_compute_edi_content_ct', compute_sudo=True)
    l10n_mx_edi_error_ct = fields.Char(copy=False)
    l10n_mx_edi_status_ct = fields.Selection(
        selection=[
            ('to_send', 'To Send'),
            ('sent', 'Sent'),
            ('to_cancel', 'To Cancel'),
        ...
        string='MX EDI status CT',
        copy=False)
    l10n_mx_edi_sat_status_ct = fields.Selection(
        selection=[
            ('valid', 'Valid'),
            ('cancelled', 'Cancelled'),
            ('not_found', 'Not Found'),
        ...
        string='SAT Status CT',
        copy=False)
    l10n_mx_edi_cfdi_name_ct = fields.Char(string='CFDI name CT', copy=False, readonly=True,
        help='The attachment name of the CFDI.')
    l10n_mx_edi_cfdi_uuid_ct = fields.Char('Fiscal Folio CT', copy=False)
    l10n_mx_edi_cfdi_ct = fields.Binary(string='Cfdi content CT', copy=False, readonly=True,
        help='The cfdi xml content encoded in base64.',
        compute='_compute_cfdi_values_ct')
    l10n_mx_edi_origin_ct = fields.Char(
        string='CFDI Origin CT',
        copy=False,
        help="Specify the existing Fiscal Folios to replace. Prepend with '04|'")
    
    l10n_mx_edi_transport_type_ct = fields.Selection(
        selection=[
            ('00', 'Carta Traslado'),
        ],
        string='Transport Type',
        ...
        help='Specify the transportation method. The Delivery Guide will contain the Complemento Carta Porte only when'
             ' federal transport is used')
    l10n_mx_edi_cfdi_file_id_ct = fields.Many2one('ir.attachment', string='CFDI file', copy=False)
    

    @api.depends('l10n_mx_edi_status_ct')
    def _l10n_mx_edi_compute_edi_content_ct(self):
        for picking in self:
            picking.l10n_mx_edi_content_ct = base64.b64encode(picking._l10n_mx_edi_create_delivery_guide_ct())


    def l10n_mx_edi_action_clear_error_ct(self):
        for record in self:
            record.l10n_mx_edi_error_ct = False


    def l10n_mx_edi_action_download_ct(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_url',
            'url':  '/web/content/stock.picking/%s/l10n_mx_edi_content_ct' % self.id
        }
    
    def _l10n_mx_edi_check_required_data_ct(self):
        for record in self:
            if not record.l10n_mx_edi_transport_type_ct:
                raise UserError(_('You must select a transport type to generate the delivery guide'))
            if record.move_line_ids.mapped('product_id').filtered(lambda product: not product.l10n_mx_edi_code_sat_id):
                raise UserError(_('All products require a SAT Code'))
            if not record.company_id.l10n_mx_edi_certificate_ids.sudo().get_valid_certificate():
    ...
    def _compute_l10n_mx_edi_cancel_picking_id_ct(self):
        for pick in self:
            if pick.l10n_mx_edi_cfdi_uuid_ct:
                replaced_pick = pick.search(
                    [('l10n_mx_edi_origin_ct', 'like', '04|%'),
                     ('l10n_mx_edi_origin_ct', 'like', '%' + pick.l10n_mx_edi_cfdi_uuid_ct + '%'),
                     ('company_id', '=', pick.company_id.id)],
                    limit=1,
                )
                pick.l10n_mx_edi_cancel_picking_id_ct = replaced_pick
            else:
                pick.l10n_mx_edi_cancel_picking_id_ct = None
    

    # -------------------------------------------------------------------------
    # XML
    # -------------------------------------------------------------------------
    def _l10n_mx_edi_create_delivery_guide_ct(self):
        def format_float(val, digits=2):
            return '%.*f' % (digits, val)
        ...
        for record in self:
            ...
            warehouse_zip = record.picking_type_id.warehouse_id.partner_id and record.picking_type_id.warehouse_id.partner_id.zip or record.company_id.zip
            origin_type, origin_uuids = None, []
            if record.l10n_mx_edi_origin_ct and '|' in record.l10n_mx_edi_origin_ct:
                split_origin = record.l10n_mx_edi_origin_ct.split('|')
                if len(split_origin) == 2:
                    origin_type = split_origin[0]
                    origin_uuids = split_origin[1].split(',')
            values = {
            ...
            xml = self.env['ir.qweb'].render(self.env.ref('l10n_mx_edi_tuodoo_cartaporte.cfdi_cartaporte_ct'), values)
            certificate = record.company_id.l10n_mx_edi_certificate_ids.sudo().get_valid_certificate()
            if certificate:
                tree = objectify.fromstring(xml)
                tree.attrib['NoCertificado'] = certificate.serial_number
                tree.attrib['Certificado'] = certificate.sudo().get_data()[0]
                cadena = record._get_cadena_chain_ct(tree, CFDI_XSLT_CADENA)
                tree.attrib['Sello'] = certificate.sudo().get_encrypted_cadena(cadena)
                xml = etree.tostring(tree)
            return xml
        
    def _l10n_mx_edi_validate_with_xsd_ct(self, xml_str, raise_error=False):
        ''' This method checks the xml_str against the combined xsd's for CartaPorte and cfd
        It is not used on every xml generated because the XSD's contain many imports which may have performance
        implications.
        :param xml_str:     The cfdi xml string.
    ...
    def _get_cadena_chain_ct(self, tree, xslt):
        xslt_root = etree.parse(tools.file_open(xslt))
        return str(etree.XSLT(xslt_root)(tree))

    def _l10n_mx_edi_decode_cfdi_ct(self, cfdi_data=None):
        ''' Helper to extract relevant data from the CFDI to be used, for example, when printing the picking.
        :param cfdi_data:   The optional cfdi data.
        :return:            A python dictionary.
        '''
        ...
        # Get the signed cfdi data.
        if not cfdi_data:
            cfdi_data = base64.b64decode(self.l10n_mx_edi_cfdi_file_id_ct.datas)
        ...
        return {
            ...
            'sello': cfdi_node.get('sello', cfdi_node.get('Sello', 'No identificado')),
            'sello_sat': tfd_node is not None and tfd_node.get('selloSAT', tfd_node.get('SelloSAT', 'No identificado')),
            'cadena': self._get_cadena_chain_ct(cfdi_node, CFDI_XSLT_CADENA),
            'certificate_number': cfdi_node.get('noCertificado', cfdi_node.get('NoCertificado')),
            'certificate_sat_number': tfd_node is not None and tfd_node.get('NoCertificadoSAT'),
            'expedition': cfdi_node.get('LugarExpedicion'),
            'fiscal_regime': cfdi_node.Emisor.get('RegimenFiscal', ''),
    ...
    # WEB SERVICES
    # -------------------------------------------------------------------------
    def _l10n_mx_edi_request_mapbox_ct(self, url, params):
        try:
            fetched_data = requests.get(url, params=params, timeout=10)
        except Exception as e:
            raise UserError(_('Unable to connect to mapbox'))
    ...
    def l10n_mx_edi_action_set_partner_coordinates_ct(self):
        mb_token = self.env['ir.config_parameter'].sudo().get_param('web_map.token_map_box', False)
        if not mb_token:
            raise UserError(_('Please configure MapBox to use this feature'))
        for record in self:
            ...
            src_address = url_quote(src)
            url = f'{MAPBOX_GEOCODE_URL}{src_address}.json?'
            fetched_data = record._l10n_mx_edi_request_mapbox_ct(url, {'access_token': mb_token})
            res = json.loads(fetched_data.content)
            if 'features' in res:
                record.picking_type_id.warehouse_id.partner_id.partner_latitude = res['features'][0]['geometry']['coordinates'][0]
                record.picking_type_id.warehouse_id.partner_id.partner_longitude = res['features'][0]['geometry']['coordinates'][1]
    ...
    def l10n_mx_edi_action_calculate_distance_ct(self):
        mb_token = self.env['ir.config_parameter'].sudo().get_param('web_map.token_map_box', False)
        if not mb_token:
            raise UserError(_('Please configure MapBox to use this feature'))
        params = {
        ...
        for record in self:
            if record.l10n_mx_edi_src_lat and record.l10n_mx_edi_src_lon \
                and record.l10n_mx_edi_des_lat and record.l10n_mx_edi_des_lon:
                url = f'{MAPBOX_MATRIX_URL}{record.l10n_mx_edi_src_lat},{record.l10n_mx_edi_src_lon};{record.l10n_mx_edi_des_lat},{record.l10n_mx_edi_des_lon}'
                fetched_data = record._l10n_mx_edi_request_mapbox_ct(url, params)
                res = json.loads(fetched_data.content)
                if 'distances' in res:
                    record.l10n_mx_edi_distance = res['distances'][0][0] // 1000
            else:
    ...
    def l10n_mx_edi_action_cancel_delivery_guide_ct(self):
        for record in self:
            pac_name = record.company_id.l10n_mx_edi_pac
            credentials = getattr(self.env['account.move'], '_l10n_mx_edi_%s_info' % pac_name)(record.company_id, 'cancel')
            if credentials.get('errors'):
            ...
            cancel_method = getattr(self.env['account.move'], '_l10n_mx_edi_%s_cancel_service' % pac_name)
            cancelled, code, msg = cancel_method(record.l10n_mx_edi_cfdi_uuid_ct, record.company_id, credentials, record.l10n_mx_edi_cancel_picking_id.l10n_mx_edi_cfdi_uuid_ct)
            if not cancelled:
                record.l10n_mx_edi_error = msg
                continue
            ...
            message = _("The CFDI Delivery Guide has been cancelled.")
            record._message_log(body=message)
            origin = '04|' + record.l10n_mx_edi_cfdi_uuid_ct
            record.write({'l10n_mx_edi_cfdi_uuid_ct': False, 'l10n_mx_edi_error': False, 'l10n_mx_edi_status_ct': 'cancelled', 'l10n_mx_edi_origin_ct': origin})

    def l10n_mx_edi_action_send_delivery_guide_ct(self):
        self._l10n_mx_edi_check_required_data_ct()
        for record in self:
            pac_name = record.company_id.l10n_mx_edi_pac
            ...
            cfdi_str = record._l10n_mx_edi_create_delivery_guide_ct()
            cfdi_signed, code, msg = getattr(self.env['account.move'], '_l10n_mx_edi_%s_sign_service' % pac_name)(credentials, cfdi_str)
            if code == 'error' or not cfdi_signed:
                record.l10n_mx_edi_error = msg
                continue
            ...
            # == Create the attachment ==
            uuid = record._l10n_mx_edi_decode_cfdi_ct(cfdi_signed).get('uuid')
            cfdi_attachment = self.env['ir.attachment'].create({
                'name': ATTACHMENT_NAME_CT.format(uuid),
                'res_id': record.id,
                'res_model': record._name,
                'type': 'binary',
                'datas': base64.b64encode(cfdi_signed),
            ...
            record.l10n_mx_edi_cfdi_file_id_ct = cfdi_attachment.id
            record.l10n_mx_edi_cfdi_name_ct = cfdi_attachment.name

            # == Chatter ==
            message = _("The CFDI Delivery Guide has been successfully signed.")
            record._message_log(body=message, attachment_ids=cfdi_attachment.ids)
            record.write({'l10n_mx_edi_cfdi_uuid_ct': uuid, 'l10n_mx_edi_error': False, 'l10n_mx_edi_status_ct': 'sent'})

    def l10n_mx_edi_action_update_sat_status_ct(self):
        '''Synchronize both systems: Odoo & SAT to make sure the delivery guide is valid.
        '''
        for record in self:
            decoded_cfdi = record._l10n_mx_edi_decode_cfdi_ct()
            supplier_rfc = record.company_id.vat
            customer_rfc = record.partner_id.commercial_partner_id.vat
            total = decoded_cfdi.get('amount_total', 0.0)
            uuid = decoded_cfdi.get('uuid', False)
            ...
            record.l10n_mx_edi_sat_status_ct = status

    @api.model
    def l10n_mx_edi_retrieve_attachments_ct(self):
        '''Retrieve all the cfdi attachments generated for this invoice.
        :return: An ir.attachment recordset
        '''
        self.ensure_one()
        if not self.l10n_mx_edi_cfdi_name_ct:
            return []
        domain = [
            ('res_id', '=', self.id),
            ('res_model', '=', self._name),
            ('name', '=', self.l10n_mx_edi_cfdi_name_ct)]
        return self.env['ir.attachment'].search(domain)

    @api.model
    def l10n_mx_edi_retrieve_last_attachment_ct(self):
        attachment_ids = self.l10n_mx_edi_retrieve_attachments_ct()
        return attachment_ids and attachment_ids[0] or None

    @api.model
    def l10n_mx_edi_get_xml_etree_ct(self, cfdi=None):
        '''Get an objectified tree representing the cfdi.
        If the cfdi is not specified, retrieve it from the attachment.
        :param cfdi: The cfdi as string
        :return: An objectified tree
        ...
        #TODO helper which is not of too much help and should be removed
        self.ensure_one()
        if cfdi is None and self.l10n_mx_edi_cfdi_ct:
            cfdi = base64.decodebytes(self.l10n_mx_edi_cfdi_ct)
        _logger.info("Estas aquí")
        _logger.info(cfdi)
        return fromstring(cfdi) if cfdi else None
    ...
    @api.model
    def l10n_mx_edi_get_tfd_etree_ct(self, cfdi):
        '''Get the TimbreFiscalDigital node from the cfdi.
        :param cfdi: The cfdi as etree
        :return: the TimbreFiscalDigital node
        '''
    ...
    @api.model
    def _get_l10n_mx_edi_cadena_ct(self):
        self.ensure_one()
        #get the xslt path
        xslt_path = CFDI_XSLT_CADENA_TFD
        #get the cfdi as eTree
        cfdi = base64.decodebytes(self.l10n_mx_edi_cfdi_ct)
        cfdi = self.l10n_mx_edi_get_xml_etree_ct(cfdi)
        cfdi = self.l10n_mx_edi_get_tfd_etree_ct(cfdi)
        #return the cadena
        return self.l10n_mx_edi_generate_cadena_ct(xslt_path, cfdi)

    @api.model
    def l10n_mx_edi_get_et_etree_ct(self, cfdi):
        """Get the ComercioExterior node from the cfdi.
        :param cfdi: The cfdi as etree
        :return: the ComercioExterior node
        """
    ...
    @api.model
    def l10n_mx_edi_generate_cadena_ct(self, xslt_path, cfdi_as_tree):
        '''Generate the cadena of the cfdi based on an xslt file.
        The cadena is the sequence of data formed with the information contained within the cfdi.
        This can be encoded with the certificate to create the digital seal.
        Since the cadena is generated with the invoice data, any change in it will be noticed resulting in a different
    ...
    @api.depends('l10n_mx_edi_cfdi_name_ct', 'l10n_mx_edi_status_ct')
    def _compute_cfdi_values_ct(self):
        '''Fill the invoice fields from the cfdi values.
        '''
        for inv in self:
            attachment_id = inv.l10n_mx_edi_retrieve_last_attachment_ct()
            # At this moment, the attachment contains the file size in its 'datas' field because
            # to save some memory, the attachment will store its data on the physical disk.
            # To avoid this problem, we read the 'datas' directly on the disk.
            datas = attachment_id._file_read(attachment_id.store_fname) if attachment_id else None
            ...
            _logger.info("Este es attachment_id.store_fname")
            _logger.info(attachment_id.store_fname)
            inv.l10n_mx_edi_cfdi_uuid_ct = None
            if not datas:
                if attachment_id:
                    _logger.error('The CFDI attachment cannot be found')
                inv.l10n_mx_edi_cfdi_ct = None
                inv.l10n_mx_edi_cfdi_supplier_rfc = None
                inv.l10n_mx_edi_cfdi_customer_rfc = None
                inv.l10n_mx_edi_cfdi_amount = None
                continue
            inv.l10n_mx_edi_cfdi_ct = datas
            cfdi = base64.decodebytes(datas).replace(
                b'xmlns:schemaLocation', b'xsi:schemaLocation')
            tree = inv.l10n_mx_edi_get_xml_etree_ct(cfdi)
            # if already signed, extract uuid
            tfd_node = inv.l10n_mx_edi_get_tfd_etree_ct(tree)
            if tfd_node is not None:
                inv.l10n_mx_edi_cfdi_uuid_ct = tfd_node.get('UUID')
            inv.l10n_mx_edi_cfdi_amount = tree.get('Total', tree.get('total'))
            inv.l10n_mx_edi_cfdi_supplier_rfc = tree.Emisor.get(
                'Rfc', tree.Emisor.get('rfc'))
            inv.l10n_mx_edi_cfdi_customer_rfc = tree.Receptor.get(
